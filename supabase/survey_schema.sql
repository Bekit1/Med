...
-- ===========================================
-- RLS (идемпотентный — безопасно перезапускать)
-- ===========================================

ALTER TABLE health_surveys ENABLE ROW LEVEL SECURITY;

-- Удалить старые политики если существуют
DROP POLICY IF EXISTS "Users can view own surveys" ON health_surveys;
DROP POLICY IF EXISTS "Users can insert own surveys" ON health_surveys;
DROP POLICY IF EXISTS "Users can update own surveys" ON health_surveys;
DROP POLICY IF EXISTS "Admins can view all surveys" ON health_surveys;

-- SELECT: пользователь видит свои ИЛИ админ видит все
...
-- INSERT: пользователь создаёт только свои
CREATE POLICY "Users can insert own surveys"
  ON health_surveys FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- UPDATE: пользователь обновляет свои (для AI-анализа)
